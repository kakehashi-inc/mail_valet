...
Mail Valetは、GmailおよびIMAPメールアカウントを一括管理し、不要メールの効率的な整理を支援するデスクトップアプリケーションです。

### 主な機能

- **複数アカウント管理**: Gmail（OAuth2）およびIMAP（クレデンシャル認証）によるマルチアカウント対応
- **メールサンプリング**: 日数または日付範囲指定でメールを取得し、From別、Subject別、またはカスタムルールでグループ化
- **ルールベースグルーピング**: 正規表現によるルールを定義し、件名/本文パターンでメールをグループ化（AND/OR条件対応）
...
- **ゴミ箱閲覧**: ゴミ箱内のメールを閲覧し、選択削除または全件完全削除
- **ラベル/フォルダ管理**: Gmailラベル/IMAPフォルダのツリー表示と取得対象の選択
