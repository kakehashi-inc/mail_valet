...
| カラム名 | 型 | デフォルト | 説明 |
|----------|-----|----------|------|
| language | `'auto' \| 'ja' \| 'en'` | `'auto'` | 表示言語。autoはOS言語を検出 |
| theme | `'auto' \| 'light' \| 'dark'` | `'auto'` | テーマ。autoはOSテーマを検出 |
...
| provider | `MailProviderId` | メールプロバイダ識別子（現在は`'gmail'`のみ） | - |
...
## 9. accounts/{account_id}/cache/sampling.json - サンプリング結果
...
| snippet | `string` | メールの概要テキスト |
| labelIds | `string[]` | 適用されているラベルIDの配列 |
| isImportant | `boolean` | 重要マークの有無 |
| isStarred | `boolean` | スターの有無 |
| body | `string?` | メール本文（オンデマンド取得） |
| rawData | `string?` | 生データ（オンデマンド取得） |
...
## 10. accounts/{account_id}/cache/sampling_meta.json - サンプリングメタ情報
...
| groupModes | `Record<string, GroupMode>` | アカウント別グルーピングモード（`'from'` \| `'subject'`） |
...
|-------------|--------------|-------------|------|
| gcp.json | clientSecret | AES-256-GCM | マシン固有キー派生 |
| tokens.json | accessToken, refreshToken | AES-256-GCM | マシン固有キー派生 |

キー派生: ホスト名 + ユーザー名 + ソルト から PBKDF2 で256bitキーを生成。
IV: 暗号化ごとにランダム16バイト生成。暗号文と共に保存。
